...
void CreditBasedMeter::idleSlope(simtime_t delta)
{
    ...
    {
        //Onur
        this->credit += static_cast<long long int>(ceil(this->reservedBandwidth * idleDuration.dbl()));
...
void CreditBasedMeter::sendSlope(inet::EtherFrame *frame)
{
    ...
    //Onur
    this->credit -= static_cast<long long int>(ceil((inChannel->getNominalDatarate() - this->reservedBandwidth) * transmissionDuration.dbl()));
...
void CreditBasedMeter::refreshReservedBandwidthAndMaxCredit()
{
    this->reservedBandwidth = 0;
    //Onur
    unsigned long long int maxSingleStreamBandwidth = 0;
    for(std::vector<IEEE8021QciFilter*>::iterator it = streamFilters.begin(); it != streamFilters.end(); ++it)
    {
        ...
        if (streamFilter->getMeterId() == static_cast<unsigned int>(this->getIndex()))
        {
            ...
            for (std::list<uint16_t>::iterator it2 = vids.begin(); it2 != vids.end(); ++it2)
            {
                uint16_t vid = *it2;
                //Onur
                unsigned long long int streamBandwidth = this->srptable->getBandwidthForStream(streamFilter->getStreamId(), vid);
    ...
    //Onur
    this->maxCredit = static_cast<long long int>(ceil((inChannel->getNominalDatarate() - this->reservedBandwidth) * duration.dbl() * (this->maxBurst - 1)));
}
...
simtime_t CreditBasedMeter::calculateTransmissionDuration(inet::EtherFrame *frame)
{
    ...
    EV_WARN << "CreditBasedMeter::calculateTransmissionDuration frameDuration "<<frameDuration
                    << " sizeFrame->getByteLength() "<<sizeFrame->getByteLength()
                            <<" PREAMBLE_BYTES "<<PREAMBLE_BYTES
                            <<" SFD_BYTES "<<SFD_BYTES
                            <<" (INTERFRAME_GAP_BITS / 8) "<<(INTERFRAME_GAP_BITS / 8)
                            <<" at time "<<simTime()<<"\n";
...
void CreditBasedMeter::emitCredit()
{
    ...
    {
        this->emit(creditSignal, static_cast<double>(credit));
