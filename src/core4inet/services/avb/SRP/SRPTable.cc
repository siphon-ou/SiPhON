...
std::ostream& operator<<(std::ostream& os, const SRPTable::TalkerEntry& entry)
{
    os << "{TalkerAddress=" << entry.address.str() << ", Module=" << entry.module->getFullName() << ", SRClass="
            ...
            << static_cast<double>(bandwidthmultiplier*bandwidthFromSizeAndInterval(entry.framesize + static_cast<size_t>(SRP_SAFETYBYTE),
...
//Onur
unsigned long long int SRPTable::getBandwidthForModule(const cModule *module)
{
    removeAgedEntriesIfNeeded();

    //Onur
    unsigned long long int bandwidth = 0;

    //Onur
    for (std::unordered_map<unsigned long long int, ListenerTable>::iterator i = listenerTables.begin();
        ...
        for (ListenerTable::const_iterator j = table.begin(); j != table.end(); ++j)
        {
            ...
            for (ListenerList::const_iterator k = llist.begin(); k != llist.end(); ++k)
            {
                if ((*k).first == module)
                {
                    ...
                    bandwidth += bandwidthmultiplier*bandwidthFromSizeAndInterval(tentry->framesize + static_cast<size_t>(SRP_SAFETYBYTE),
...
//Onur
unsigned long long int SRPTable::getBandwidthForModuleAndSRClass(const cModule *module, SR_CLASS srClass)
{
    removeAgedEntriesIfNeeded();
        ...
        for (ListenerTable::const_iterator j = table.begin(); j != table.end(); ++j)
        {
            ...
            for (ListenerList::const_iterator k = llist.begin(); k != llist.end(); ++k)
            {
                if ((*k).first == module)
                {
                    ...
                    {
                        bandwidth += bandwidthmultiplier*bandwidthFromSizeAndInterval(
...
unsigned long long int SRPTable::getBandwidthForStream(uint64_t streamId, uint16_t vid)
{
    removeAgedEntriesIfNeeded();
    ...
    return bandwidthmultiplier*bandwidthFromSizeAndInterval(tentry->framesize + static_cast<size_t>(SRP_SAFETYBYTE), tentry->intervalFrames,
            getIntervalForClass(tentry->srClass));
}
...
std::list<uint16_t> SRPTable::getVidsForStreamId(uint64_t streamId)
{
    ...
    //Onur
    for (std::unordered_map<unsigned long long int, TalkerTable>::iterator iter = talkerTables.begin();
...
void SRPTable::printState()
{
    ...
    //Onur
    for (std::unordered_map<unsigned long long int, TalkerTable>::const_iterator i = talkerTables.begin();
            i != talkerTables.end(); ++i)
    {
        ...
        for (TalkerTable::const_iterator j = table.begin(); j != table.end(); ++j)
        {
            EV_DETAIL << (*i).first << "   " << (*j).first << "   " << (*j).second->module->getName() << "   "
                    ...
                    << static_cast<double>(bandwidthmultiplier*bandwidthFromSizeAndInterval(
    ...
    //Onur
    for (std::unordered_map<unsigned long long int, ListenerTable>::const_iterator i = listenerTables.begin();
            i != listenerTables.end(); ++i)
    {
        ListenerTable table = i->second;
        for (ListenerTable::const_iterator j = table.begin(); j != table.end(); ++j)
...
void SRPTable::clear()
{
    ...
    //Onur
    for (std::unordered_map<unsigned long long int, ListenerTable>::iterator iter = listenerTables.begin();
...
void SRPTable::removeAgedEntries()
{
    ...
    nextAging = 0;
    //Onur
    for (std::unordered_map<unsigned long long int, ListenerTable>::iterator listenerTable = listenerTables.begin();
