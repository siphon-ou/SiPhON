    ...
    if (msg->arrivedOn("in"))
    {
        ...
        else
        {
            ...
            else if (ListenerReady* listenerReady = dynamic_cast<ListenerReady*>(msg))
            {
                ...
                //Onur
                unsigned long long int utilizedBandwidth = srpTable->getBandwidthForModule(port);
                //Add Higher Priority Bandwidth
                //Onur
                utilizedBandwidth += static_cast<unsigned long long int>(port->getSubmodule("shaper")->par("AVBHigherPriorityBandwidth"));
                //Onur
                unsigned long long int requiredBandwidth = srpTable->getBandwidthForStream(listenerReady->getStreamID(),
                        listenerReady->getVlan_identifier());

                cGate *physOutGate = port->getSubmodule("mac")->gate("phys$o");
                cChannel *outChannel = physOutGate->findTransmissionChannel();

                //Onur - changed to double because unsigned long limits the bandwidth to 4,294,967,295 (2^32 - 1)
                double totalBandwidth = static_cast<double>(outChannel->getNominalDatarate());
                ...
                else
                {
                    EV_DETAIL << "Listener for stream " << listenerReady->getStreamID()
                            ...
                            << "MBit/s, totalBandwidth "
                            << static_cast<double>(totalBandwidth)
                            << " reservableBandwidth "
                            << static_cast<double>(reservableBandwidth)
                            << " utilizedBandwidth "
                            << static_cast<double>(utilizedBandwidth)
                            << " channel name "
                            << outChannel->getName()
                            << " physOutGate name "
                            << physOutGate->getName()
